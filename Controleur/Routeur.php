...
class Routeur {
    ...
    public function routerRequete() {
    	try { 		
	        if (isset($_GET['action'])) {
    			if (Securite::utilisateurConnected() === '1' || $_GET['action'] == 'connexion') {	   
	            	if ($_GET['action'] == 'accueil') {
	                    $this->ctrlAccueil->accueil();
	                } else if ($_GET['action'] == 'connexion') { 
	                	$pwd = $this->getParametre($_POST, 'mot_de_passe');
	                    $this->ctrlAccueil->logInUser($pwd);
			    	} else if ($_GET['action'] == 'deconnexion') {
	                    $this->ctrlAccueil->logOutUser(); 
	            	}
	...
	//Partie Menu Organisation -> Generation des poules
	            		...
	            		$categorie = $this->getParametre($_POST, 'categorie');
	            		$this->ctrlOrganisation->genererExcelPoule($categorie);
	            	}
	...
	//Erreur : parametre action non valide          
	            	...
	            	$erreur[] = "Merci de vous connecter";
            	...
            	session_destroy();
            	//$this->ctrlAccueil->accueil();
            	$this->ctrlAccueil->afficheFormulaire();
            }
        } catch (Exception $e) {
